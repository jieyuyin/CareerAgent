...
import { confirmAgentAction, getAgentMessages, getRewriteSuggestion, getToolExecutions, rejectAgentAction, reviewRewriteSuggestion, streamAgentMessage } from '../api/agent.js'
...
export default function DevAgentPage() {
  ...
  async function submit(event) {
    ...
    try {
      let activeId=conversationId;setStreamed('');await streamAgentMessage({message:input,conversationId,onEvent:(event,data)=>{if(event==='conversation'){activeId=data.conversationId;setConversationId(activeId)}else if(event==='message_delta')setStreamed(value=>value+String(data.data||''));else if(event==='confirmation_required')setPending(data.data||null)}})
      if(activeId)await refresh(activeId);setStreamed('');setInput('')
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="min-h-80 space-y-3">
            ...
            {streamed&&<div className="mr-12 rounded-xl bg-slate-100 p-3"><b className="text-xs text-slate-500">ASSISTANT</b><p className="mt-1 whitespace-pre-wrap break-words">{streamed}</p></div>}
          </div>
